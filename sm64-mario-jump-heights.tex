...
We define a function $v_g(t): \mathbb{R}^{+} \to \mathbb{R}$, such that
$v_g(t) = max(v_0 - g \lfloor t \rfloor, -75)$.  This function will represent
Mario's velocity $t$ in-game ticks after entering an airborne state.  We take
the max because Mario's terminal velocity is 75 units downward per frame.  We
also take the floor of $t$ so we can have a continuous domain.\\
...
To solve this, we will use integral Calculus, but with some minor liberties.
Since we're using the floor function and the max function, our function is not
continuous over its domain or range, thus can't be integrated normally.  To
solve this, we will do the following:
...
This function has the nice property that it's continuous over both its domain
and range, as well as having a linear slope which more accurately interpolates
the non-integer values.  This also ensures the function is monotonic for
$x \geq 0$, which is not true with a purely quadratic antiderivative.\\

Next, we'll redefine $v_g(t)$ to be piecewise, free of the max function.  This
will allow us to integrate each component of the function which is continuous
over its own domain, and combine together at the end.
...
&=& \begin{cases}
   ...
   -75 \lfloor t \rfloor & v_0 - g \lfloor t \rfloor \leq -75 \\
\end{cases} \\
&=& \begin{cases}
...
Lastly, we will find an inverse for for the height function
...
\begin{eqnarray*}
v &:& A \to \mathbb{R}^{+} \text{ such that } A \subseteq \mathbb{R}, 0 \in \text{\textsf{Im}}(v) \\
v^{-1} &:& \mathbb{R}^{+} \to A \text{ such that for } \forall r \in \text{\textsf{Im}}(v), \exists a \in A: v(a) = r \\
\end{eqnarray*}
...
With this knowledge, we now know that if we can find $n$ in terms of $r$, we can
then rewrite $h_g (v(a)) = m_n v(a) + b_n$ over the interval
...
n &=& \frac{-1 + \sqrt{\frac{g}{g} + \frac{8r}{g}}}{2} \\
n &=& \frac{-1 + \sqrt{\frac{8r + g}{g}}}{2} \\
n &=& \frac{-1 + \frac{\sqrt{8r + g}}{\sqrt{g}}}{2} \\
n &=& \frac{\frac{-\sqrt{g}}{\sqrt{g}} + \frac{\sqrt{8r + g}}{\sqrt{g}}}{2} \\
n &=& \frac{\frac{\sqrt{8r + g} - \sqrt{g}}{\sqrt{g}}}{2} \\
n &=& \frac{\sqrt{8r + g} - \sqrt{g}}{2 \sqrt{g}} \\
n &=& \frac{\sqrt{g} (\sqrt{8r + g} - \sqrt{g})}{\sqrt{g} (2 \sqrt{g})} \\
n &=& \frac{\sqrt{g} \sqrt{8r + g} - g}{2g} \\
n &=& \frac{\sqrt{8rg + g^2} - g}{2g} \\
\end{eqnarray*}

\begin{eqnarray*}
n &=& \frac{\sqrt{8rg + g^2}}{2g} - \frac{1}{2} \\
n &=& \frac{\sqrt{8rg + g^2}}{2 \sqrt{g^2}} - \frac{1}{2} \\
n &=& \frac{1}{2} \sqrt{\frac{8r}{g} + 1} - \frac{1}{2} \\
n &=& \frac{1}{\sqrt{4}} \sqrt{\frac{8r}{g} + 1} - \frac{1}{2} \\
n &=& \sqrt{\frac{8r}{4g} + \frac{1}{4}} - \frac{1}{2} \\
n &=& \sqrt{\frac{2r}{g} + \frac{1}{4}} - \frac{1}{2} \\
\end{eqnarray*}

This will give us a real number which is between our desired value for $n$.  We
get the exact value for $n$ by taking the floor of the above.  This results in
...
Now, we're going to solve for $m_n$ and $b_n$ in terms of $n$.  This is
actually pretty simple.  We just plug in for the endpoint of the $n$'th interval
for all the ceil terms and write it in the correct form.\\
...
Now, let's revisit our previous solution for $v^{-1}(h^{-1}_g(r))$ plug in for $m_n$ and $b_n$.\\
...
Now, we could just plug in for $n$, but that would result in a butt-ugly
equation with no way to simplify.  Instead, we're going to define
...
\hline
Name & Identifier & Domain and Range & Definition \\
\hline
...
derived height function $h(v,g)$ to compute the peak height for each
jumping action.
...
\hline
Single Jump & \textsf{ACT\_SINGLE\_JUMP} & $\lceil \frac{168 + f}{16} \rceil (44 + \frac{f}{4} - 2\lceil \frac{168 + f}{16} \rceil)$ & $\lceil \frac{168 + f}{32} \rceil (23 + \frac{f}{8} - 2\lceil \frac{168 + f}{32} \rceil)$ & $f \geq -184$ \\
Double Jump & \textsf{ACT\_DOUBLE\_JUMP} & $\lceil \frac{208 + f}{16} \rceil (54 + \frac{f}{4} - 2\lceil \frac{208 + f}{16} \rceil)$ & $\lceil \frac{208 + f}{32} \rceil (28 + \frac{f}{8} - 2\lceil \frac{208 + f}{32} \rceil)$ & $f \geq -224$ \\
Jump With Object & \textsf{ACT\_HOLD\_JUMP} & $\lceil \frac{168 + f}{16} \rceil (44 + \frac{f}{4} - 2\lceil \frac{168 + f}{16} \rceil)$ & $\lceil \frac{168 + f}{32} \rceil (23 + \frac{f}{8} - 2\lceil \frac{168 + f}{32} \rceil)$ & $f \geq -184$ \\
Jump While Riding Shell & \textsf{ACT\_RIDING\_SHELL\_JUMP} & $\lceil \frac{168 + f}{16} \rceil (44 + \frac{f}{4} - 2\lceil \frac{168 + f}{16} \rceil)$ & $\lceil \frac{168 + f}{32} \rceil (23 + \frac{f}{8} - 2\lceil \frac{168 + f}{32} \rceil)$ & $f \geq -184$ \\
Sliding Kick (landing bounce) & \textsf{ACT\_SLIDE\_KICK} & $\lfloor \frac{v}{4} \rfloor (\frac{v}{2} - 1 - \lfloor \frac{v}{4} \rfloor)$ & & $-75 \leq v \leq 0$\\
Steep Jump & \textsf{ACT\_STEEP\_JUMP} & $\lceil \frac{168 + f}{16} \rceil (44 + \frac{f}{4} - 2\lceil \frac{168 + f}{16} \rceil)$ & $\lceil \frac{168 + f}{32} \rceil (23 + \frac{f}{8} - 2\lceil \frac{168 + f}{32} \rceil)$ & $f \geq -184$ \\
\hline
\end{tabular}
\end{tiny}
\end{center}
...
Also notice that we use the floor function instead of ceil function for the
sliding kick landing bounce.  This is because given $x \in \mathbb{R}$,
$\lceil -x \rceil = - \lfloor x \rfloor$ is generally true, and this fact was
used in the derivation.\\

We're now going to show the required speed to reach a jump height of
$r \in \mathbb{R}^{+}$ for all the variable jumping actions.  To every single
action being a nightmare equation, we're going to refer to the definition of the
inverse height function $h_g^{-1}$ from
\hyperref[sec:height-function-inverse-height]{section 1.4}.  Also note that we
don't provide a function-specific domain here.  This is because it's redundant,
as the domain of $h_g^{-1}$ is always $\mathbb{R}^{+}$ by definition.\\
...
$h^j \in \{h_g : g \in \mathbb{R}^{+} - \{0\}\}$ for jumping action $j$, $h^j$ can be
classified into one of the following complexity classes:
...
vertical speed belong to $\Theta(n^2)$.
...
However, ceilings can only be grabbed either from a single jump or double jump
state.\\
